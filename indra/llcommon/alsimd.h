...
// SKOOMA-PORT: __builtin_shufflevector arrived in GCC 12; our Linux build is GCC 11, so it takes
// the intrinsic backend, which Alchemy keeps equivalent for exactly this comparison.
#if defined(__GNUC__) && !defined(__clang__) && __GNUC__ < 12 && !defined(AL_SIMD_FORCE_INTRINSICS)
    #define AL_SIMD_FORCE_INTRINSICS 1
#endif

...
public:
    ...
    explicit LLVector4(const glm::vec3& vec) { mV[VX] = vec.x; mV[VY] = vec.y; mV[VZ] = vec.z; mV[VW] = 1.f; }
    explicit LLVector4(const glm::vec4& vec) { mV[VX] = vec.x; mV[VY] = vec.y; mV[VZ] = vec.z; mV[VW] = vec.w; }
    explicit operator glm::vec3() const { return glm::vec3(mV[VX], mV[VY], mV[VZ]); }
    explicit operator glm::vec4() const { return glm::make_vec4(mV); }
    void set(const glm::vec4& vec) { mV[VX] = vec.x; mV[VY] = vec.y; mV[VZ] = vec.z; mV[VW] = vec.w; }
    void set(const glm::vec3& vec, F32 w = 1.f) { mV[VX] = vec.x; mV[VY] = vec.y; mV[VZ] = vec.z; mV[VW] = w; }

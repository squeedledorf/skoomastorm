...
void ALPanelFloater::onClose(bool app_quitting)
{
    ...
    if (!app_quitting)
    {
        if (ALDockPanel* pane = mPane.get() ? ALViewType::as<ALDockPanel>(mPane.get()) : nullptr)
        {
            pane->dock();
        }
    }
...
void ALDockPanel::popOut()
{
    ...
    if (LLLayoutPanel* holder = ALViewType::as<LLLayoutPanel>(home))
    {
        if (LLLayoutStack* stack = ALViewType::as<LLLayoutStack>(holder->getParent()))
        {

...
namespace
{
    ...
    std::string_view hexOf(hex_buf_t& buf, const LLColor4& color)
    {
        ...
        const int n = snprintf(buf.data(), buf.size(), "#%02x%02x%02x",
                               byte(color.mV[VRED]), byte(color.mV[VGREEN]), byte(color.mV[VBLUE]));
        return std::string_view(buf.data(), llclamp(n, 0, (int)buf.size() - 1));
    }
}
...
void ALColorPicker::drawChannels() const
{
    ...
    hex_buf_t hex;
    font->renderUTF8(std::string(hexOf(hex, mColor)), 0, mChannels.mRight, mChannels.mBottom + 2, ink.get(),

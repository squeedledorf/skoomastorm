...
#include "alviewtype.h" // SKOOMA-PORT: Alchemy's llview.h brings this in

namespace
{
    // SKOOMA-PORT: our LLUI has no getSettingGroup()
    LLControlGroup* configSettings()
    {
        const auto& groups = LLUI::getInstance()->mSettingGroups;
        const auto found = groups.find("config");
...
bool ALStudioFloater::handleUndoKeys(KEY key, MASK mask)
{
    ...
    LLView* text_view = dynamic_cast<LLView*>(text); // SKOOMA-PORT: no LLEditMenuHandler::asView()
...
void ALStudioFloater::saveState()
{
    ...
    if (LLControlGroup* settings = configSettings())
    {
...
{
    LLControlGroup* settings = configSettings();

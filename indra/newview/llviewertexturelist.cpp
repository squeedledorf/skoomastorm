                                                   ...
                                                   const LLUUID& force_id)
{
    ...
    std::string full_path = gDirUtilp->findSkinnedFilename("textures", std::string(filename));
    if (full_path.empty())
    {
                                                   ...
                                                   LLHost request_from_host)
{
    ...
    // SKOOMA-PORT: the setAllowCompression(false) call for BOOST_SCULPTED is gone with the GL
    // texture-compression path; nothing compresses sculpt maps any more. The fast cache
    // exclusion for sculpts below still applies.

...
void LLSurfacePatch::disconnectNeighbor(LLSurface *surfacep)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        // The edge flags are cleared while the neighbour is still known; the old code cleared
        // the neighbour first, so its edge checks afterwards never matched.
...
// <FS:CR> Aurora Sim
    // The other two buffer corners at a region edge with a mismatched var region beside it:
    // the diagonal neighbour is the only one holding the shared sample. These ran inside
    // updateNormals' west and south edge passes before the CPU terrain LOD was removed.
...
// <FS:CR> Aurora Sim
                    ...
                    *corner = *(getNeighborPatch(EAST)->mDataZ + (getNeighborPatch(EAST)->getSurface()->getGridsPerPatchEdge() - 1)*getNeighborPatch(EAST)->getSurface()->getGridsPerEdge());
// </FS:CR> Aurora Sim
                }
                else
                {
...
// <FS:CR> Aurora Sim
                    ...
                    *corner = *(getNeighborPatch(NORTH)->mDataZ + (getNeighborPatch(NORTH)->getSurface()->getGridsPerPatchEdge() - 1));
// </FS:CR> Aurora Sim
                }
                else
                {
...
void LLSurfacePatch::dirtyZ()
{
    ...
    dirty();
    // SKOOMA-PORT: every height write ends here -- the LayerData decode and the SS region
    // object cache's terrain restore alike -- so the GPU surface maps are staled here rather
    // than only in decompressDCTPatch.
    mSurfacep->dirtySurfaceMaps();

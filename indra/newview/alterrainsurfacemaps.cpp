...
void ALTerrainSurfaceMaps::refresh()
{
    ...
    // PORT-TODO: sized from the region, so OpenSim var regions work at any multiple of 16 m, but
    // the whole map is rebuilt and re-uploaded on every dirty frame. An 8192 m region is 8197^2
    // texels (~270 MB R32F + ~270 MB RG16F, plus ~800 MB of F32 staging) and a full rebuild per
    // frame while LayerData streams in. Large var regions want dirty-rect updates (per patch,
    // glTexSubImage2D) and a GL_MAX_TEXTURE_SIZE check; not done yet.

    bool has_neighbor[8];
    for (U32 dir = 0; dir < 8; ++dir)
    {
        // <FS:TJ> [FIRE-36100] A direction holds a list of neighbours on var-region grids.
        has_neighbor[dir] = !mSurface.mNeighbors[dir].empty();
    }

...
{
    static const std::vector<std::string> LUT_EXTENSIONS = { "cube", "tga", "png", "jpg", "jpeg", "bmp" }; // SKOOMA-PORT: no webp decoder
...
void ALFloaterLightBox::refreshBypassBadge()
{
    ...
    {
        LLStringUtil::format_map_t args;
        args["[COUNT]"] = std::to_string(bypassed);
...
void ALFloaterLightBox::fillHistoryList()
{
    ...
    for (size_t index = 0; index < mHistory.depth(); ++index)
    {
        ...
        if (step.what.empty())
        {
            ...
            {
                LLStringUtil::format_map_t args;
                args["[COUNT]"] = std::to_string(changes.size());
